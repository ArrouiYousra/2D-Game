...
public class Lwjgl3Launcher {
    ...
    private static Lwjgl3Application createApplication() {
        return new Lwjgl3Application(new TwoDGame(), getDefaultConfiguration());
    }

    private static Lwjgl3ApplicationConfiguration getDefaultConfiguration() {
        Lwjgl3ApplicationConfiguration configuration = new Lwjgl3ApplicationConfiguration();
        configuration.setTitle("2D-Game");
        //// Vsync limits the frames per second to what your hardware can display, and helps eliminate
        //// screen tearing. This setting doesn't always work on Linux, so the line after is a safeguard.
        configuration.useVsync(true);
        //// Limits FPS to the refresh rate of the currently active monitor, plus 1 to try to match fractional
        //// refresh rates. The Vsync setting above should limit the actual FPS to match the monitor.
        // MODIFIÉ : Éviter getDisplayMode() qui peut bloquer dans WSL - utiliser un FPS fixe
        // Si vous n'êtes pas dans WSL et que ça fonctionnait hier, décommentez la ligne suivante :
        // configuration.setForegroundFPS(Lwjgl3ApplicationConfiguration.getDisplayMode().refreshRate + 1);
        configuration.setForegroundFPS(60); // FPS fixe pour éviter le blocage dans WSL
